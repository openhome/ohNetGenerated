...
interface IDvProviderLinnCoUkExakt22
{
     ...
     *
     */
    public boolean setPropertyTestTrackPlaying(boolean aValue);
 ...
 */
public class DvProviderLinnCoUkExakt22 extends DvProvider implements IDvProviderLinnCoUkExakt22
{
    ...
    private IDvInvocationListener iDelegateGetTestTrackPlaying;
    ...
    {
        super(aDevice, "linn.co.uk", "Exakt2", 2);
    }
    ...
    public boolean getPropertyTestTrackPlaying()
    {
        return iPropertyTestTrackPlaying.getValue();
    }
     ...
     *
     * <p>The action's availability will be published in the device's service.xml.
     * GetTestTrackPlaying must be overridden if this is called.
     */      
    protected void enableActionGetTestTrackPlaying()
    {
        Action action = new Action("GetTestTrackPlaying");
        action.addOutputParameter(new ParameterRelated("TestTrackPlaying", iPropertyTestTrackPlaying));
        iDelegateGetTestTrackPlaying = new DoGetTestTrackPlaying();
        enableAction(action, iDelegateGetTestTrackPlaying);
    }
     ...
     *
     * <p>Must be implemented iff {@link #enableActionGetTestTrackPlaying} was called.</remarks>
     *
     * @param aInvocation   Interface allowing querying of aspects of this particular action invocation.</param>
     */
    protected boolean getTestTrackPlaying(IDvInvocation aInvocation)
    {
        throw (new ActionDisabledError());
    }
    ...
    private class DoGetTestTrackPlaying implements IDvInvocationListener
    {
        public void actionInvoked(long aInvocation)
        {
            DvInvocation invocation = new DvInvocation(aInvocation);
            boolean testTrackPlaying;
            try
            {
                invocation.readStart();
                invocation.readEnd();
                 testTrackPlaying = getTestTrackPlaying(invocation);
            }
            catch (ActionError ae)
            {
                invocation.reportActionError(ae, "GetTestTrackPlaying");
                return;
            }
            catch (PropertyUpdateError pue)
            {
