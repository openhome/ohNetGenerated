...
{
    public interface IDvProviderLinnCoUkCloud2 : IDisposable
    {
        ...
        /// <param name="aValue">New value for the property</param>
        /// <returns>true if the value has been updated; false if aValue was the same as the previous value</returns>
        bool SetPropertyAccountId(string aValue);

        /// <summary>
        /// Get a copy of the value of the AccountId property
        /// </summary>
        /// <returns>Value of the AccountId property.</param>
    ...
    /// </summary>
    public class DvProviderLinnCoUkCloud2 : DvProvider, IDisposable, IDvProviderLinnCoUkCloud2
    {
        ...
        private ActionDelegate iDelegateGetAccountId;
        ...
        private PropertyString iPropertyAccountId;
        ...
        protected DvProviderLinnCoUkCloud2(DvDevice aDevice)
            : base(aDevice, "linn.co.uk", "Cloud", 2)
        {
        ...
        /// </summary>
        public void EnablePropertyAccountId()
        {
            List<String> allowedValues = new List<String>();
            iPropertyAccountId = new PropertyString(new ParameterString("AccountId", allowedValues));
            AddProperty(iPropertyAccountId);
        }
        ...
        /// Set the value of the AccountId property
        /// </summary>
        /// <remarks>Can only be called if EnablePropertyAccountId has previously been called.</remarks>
        /// <param name="aValue">New value for the property</param>
        /// <returns>true if the value has been updated; false if aValue was the same as the previous value</returns>
        public bool SetPropertyAccountId(string aValue)
        {
            if (iPropertyAccountId == null)
                throw new PropertyDisabledError();
        ...
        public string PropertyAccountId()
        {
            if (iPropertyAccountId == null)
                throw new PropertyDisabledError();
            return iPropertyAccountId.Value();
        }
        ...
        /// </summary>
        /// <remarks>The action's availability will be published in the device's service.xml.
        /// GetAccountId must be overridden if this is called.</remarks>
        protected void EnableActionGetAccountId()
        {
            OpenHome.Net.Core.Action action = new OpenHome.Net.Core.Action("GetAccountId");
            action.AddOutputParameter(new ParameterRelated("AccountId", iPropertyAccountId));
            iDelegateGetAccountId = new ActionDelegate(DoGetAccountId);
            EnableAction(action, iDelegateGetAccountId, GCHandle.ToIntPtr(iGch));
        }
        ...
        ///
        /// Must be implemented iff EnableActionGetAccountId was called.</remarks>
        /// <param name="aInvocation">Interface allowing querying of aspects of this particular action invocation.</param>
        /// <param name="aAccountId"></param>
        protected virtual void GetAccountId(IDvInvocation aInvocation, out string aAccountId)
        {
            throw (new ActionDisabledError());
        }
        ...
        private static int DoGetChallengeResponse(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            DvProviderLinnCoUkCloud2 self = (DvProviderLinnCoUkCloud2)gch.Target;
        ...
        private static int DoGetAccountId(IntPtr aPtr, IntPtr aInvocation)
        {
            GCHandle gch = GCHandle.FromIntPtr(aPtr);
            DvProviderLinnCoUkCloud2 self = (DvProviderLinnCoUkCloud2)gch.Target;
            DvInvocation invocation = new DvInvocation(aInvocation);
            ...
            try
            {
                invocation.ReadStart();
                invocation.ReadEnd();
                self.GetAccountId(invocation, out accountId);
            }
            catch (ActionError e)
            {
                invocation.ReportActionError(e, "GetAccountId");
                return -1;
            }
            catch (PropertyUpdateError)
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", new object[] { "GetAccountId" }));
                return -1;
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("WARNING: unexpected exception {0} thrown by {1}", new object[] { e, "GetAccountId" });
                System.Diagnostics.Debug.WriteLine("         Only ActionError or PropertyUpdateError should be thrown by actions");
                return -1;
            }
            try
            {
                invocation.WriteStart();
                invocation.WriteString("AccountId", accountId);
                invocation.WriteEnd();
            }
            catch (ActionError)
            {
