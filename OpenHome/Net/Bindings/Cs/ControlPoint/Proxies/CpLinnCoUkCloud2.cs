...
{
    public interface ICpProxyLinnCoUkCloud2 : ICpProxy, IDisposable
    {
        ...
        void SyncGetAccountId(out String aAccountId);
        void BeginGetAccountId(CpProxy.CallbackAsyncComplete aCallback);
        void EndGetAccountId(IntPtr aAsyncHandle, out String aAccountId);
        ...
        void SetPropertyAccountIdChanged(System.Action aAccountIdChanged);
        String PropertyAccountId();
        void SetPropertyPublicKeyChanged(System.Action aPublicKeyChanged);
        String PropertyPublicKey();
    }

    internal class SyncGetChallengeResponseLinnCoUkCloud2 : SyncProxyAction
    {
    ...
    internal class SyncSetAssociatedLinnCoUkCloud2 : SyncProxyAction
    {
        private CpProxyLinnCoUkCloud2 iService;
    ...
    internal class SyncGetConnectedLinnCoUkCloud2 : SyncProxyAction
    {
        private CpProxyLinnCoUkCloud2 iService;
    ...
    internal class SyncGetAccountIdLinnCoUkCloud2 : SyncProxyAction
    {
        private CpProxyLinnCoUkCloud2 iService;
        ...
        protected override void CompleteRequest(IntPtr aAsyncHandle)
        {
            iService.EndGetAccountId(aAsyncHandle, out iAccountId);
        }
    };

    internal class SyncGetPublicKeyLinnCoUkCloud2 : SyncProxyAction
    {
        private CpProxyLinnCoUkCloud2 iService;
    ...
    /// </summary>
    public class CpProxyLinnCoUkCloud2 : CpProxy, IDisposable, ICpProxyLinnCoUkCloud2
    {
        ...
        private OpenHome.Net.Core.Action iActionGetAccountId;
        ...
        private System.Action iAccountIdChanged;
        ...
        public CpProxyLinnCoUkCloud2(ICpDevice aDevice)
            : base("linn-co-uk", "Cloud", 2, aDevice)
        {
            ...
            iActionGetAccountId = new OpenHome.Net.Core.Action("GetAccountId");
            ...
            iAccountId = new PropertyString("AccountId", AccountIdPropertyChanged);
        ...
        /// <remarks>Blocks until the action has been processed
        /// on the device and sets any output arguments</remarks>
        /// <param name="aAccountId"></param>
        public void SyncGetAccountId(out String aAccountId)
        {
            SyncGetAccountIdLinnCoUkCloud2 sync = new SyncGetAccountIdLinnCoUkCloud2(this);
            BeginGetAccountId(sync.AsyncComplete());
            sync.Wait();
            sync.ReportError();
            aAccountId = sync.AccountId();
        }
        ...
        /// <remarks>Returns immediately and will run the client-specified callback when the action
        /// later completes.  Any output arguments can then be retrieved by calling
        /// EndGetAccountId().</remarks>
        /// <param name="aCallback">Delegate to run when the action completes.
        /// This is guaranteed to be run but may indicate an error</param>
        public void BeginGetAccountId(CallbackAsyncComplete aCallback)
        {
            Invocation invocation = iService.Invocation(iActionGetAccountId, aCallback);
            int outIndex = 0;
            invocation.AddOutput(new ArgumentString((ParameterString)iActionGetAccountId.OutputParameter(outIndex++)));
            iService.InvokeAction(invocation);
        }
        ...
        /// <param name="aAsyncHandle">Argument passed to the delegate set in the above Begin function</param>
        /// <param name="aAccountId"></param>
        public void EndGetAccountId(IntPtr aAsyncHandle, out String aAccountId)
        {
            uint code;
            string desc;
            if (Invocation.Error(aAsyncHandle, out code, out desc))
            ...
            uint index = 0;
            aAccountId = Invocation.OutputString(aAsyncHandle, index++);
        }
        ...
        /// CpProxyLinnCoUkCloud2 instance will not overlap.</remarks>
        ...
        /// <remarks>Callbacks may be run in different threads but callbacks for a
        /// CpProxyLinnCoUkCloud2 instance will not overlap.</remarks>
        /// <param name="aAccountIdChanged">The delegate to run when the state variable changes</param>
        public void SetPropertyAccountIdChanged(System.Action aAccountIdChanged)
        {
            lock (iPropertyLock)
            {
        ...
        /// called and a first eventing callback received more recently than any call
        /// to Unsubscribe().</remarks>
        /// <returns>Value of the AccountId property</returns>
